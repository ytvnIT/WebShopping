...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.bestdeals_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.bestdeals_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && bestdeals_get_cookie('to_demo_fixed')==1) bestdeals_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.bestdeals_options_field_info .to_demo_pin', function(e) {
		bestdeals_options_fix_demo(jQuery(this).parents('.bestdeals_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	bestdeals_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content .bestdeals_options_input,#partition_schemes_content .bestdeals_options_field input[type="hidden"]').change(function(e) {
		"use strict";
		var tab = jQuery(this).parents('.bestdeals_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		bestdeals_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content .bestdeals_options_input_color').blur(function(e) {
		"use strict";
		var tab = jQuery(this).parents('.bestdeals_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .bestdeals_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.bestdeals_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? BESTDEALS_GLOBALS['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.bestdeals_options_tab').find('> .bestdeals_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="bestdeals_options_schemes_order" class="bestdeals_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function bestdeals_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.bestdeals_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.bestdeals_options').find('>.bestdeals_options_field_info');
...
function bestdeals_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.bestdeals_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	bestdeals_message_confirm(BESTDEALS_GLOBALS['to_strings']['scheme_delete_confirm'], BESTDEALS_GLOBALS['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'bestdeals_options_scheme_delete',
			nonce: BESTDEALS_GLOBALS['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.bestdeals_options_tab').find('> .bestdeals_options_schemes_order').val()
		};
		setTimeout(function(){
			bestdeals_message_info(BESTDEALS_GLOBALS['to_strings']['recompile_styles'], BESTDEALS_GLOBALS['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(BESTDEALS_GLOBALS['ajax_url'], data, function(response) {
			"use strict";
			var rez = JSON.parse(response);
			if (rez.error === '') {
				bestdeals_message_success(BESTDEALS_GLOBALS['to_strings']['scheme_delete_complete']+'<br>'+BESTDEALS_GLOBALS['to_strings']['reload_page'], BESTDEALS_GLOBALS['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				bestdeals_message_warning(BESTDEALS_GLOBALS['to_strings']['scheme_delete_failed']+':<br>'+rez.error, BESTDEALS_GLOBALS['to_strings']['scheme_delete']);
			}
		});
...
function bestdeals_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.bestdeals_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	bestdeals_message_confirm(BESTDEALS_GLOBALS['to_strings']['scheme_copy_confirm'], BESTDEALS_GLOBALS['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'bestdeals_options_scheme_copy',
			nonce: BESTDEALS_GLOBALS['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.bestdeals_options_tab').find('> .bestdeals_options_schemes_order').val()
		};
		...
		jQuery.post(BESTDEALS_GLOBALS['ajax_url'], data, function(response) {
			...
			var rez = JSON.parse(response);
			if (rez.error === '') {
				bestdeals_message_success(BESTDEALS_GLOBALS['to_strings']['scheme_copy_complete']+'<br>'+BESTDEALS_GLOBALS['to_strings']['reload_page'], BESTDEALS_GLOBALS['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				bestdeals_message_warning(BESTDEALS_GLOBALS['to_strings']['scheme_copy_failed']+':<br>'+rez.error, BESTDEALS_GLOBALS['to_strings']['scheme_copy']);
			}
		});
...
function bestdeals_options_fix_demo(info) {
	var info2 = info.parents('.bestdeals_options').toggleClass('to_demo_fixed').find('>.bestdeals_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.bestdeals_options').append(clonedObj);
		info2 = info.parents('.bestdeals_options').find('>.bestdeals_options_field_info');
	}
	bestdeals_set_cookie('to_demo_fixed', jQuery('.bestdeals_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function bestdeals_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.bestdeals_options_field_info').data('scheme');
